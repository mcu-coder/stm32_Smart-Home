...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuehumidness != cur->valuehumidness)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuehumidness Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flaghumidness = 1;
            ret = 1;
        }
    }
    ...
    if(last->valuehPa != cur->valuehPa)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuehPa Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flaghPa = 1;
            ret = 1;
        }
    }
    if(last->valuesmoke != cur->valuesmoke)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            ...
            gizwitsProtocol.waitReportDatapointFlag.flagsmoke = 1;
            ret = 1;
        }
    }
    ...
    if(last->valueAQI != cur->valueAQI)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueAQI Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagAQI = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flaghumidness)
    {
        devStatusTmp.valuehumidness = gizY2X(humidness_RATIO,  humidness_ADDITION, dataPoints->valuehumidness); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuehumidness,humidness_LEN);
        byteFieldOffset += humidness_LEN;
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flaghPa)
    {
        devStatusTmp.valuehPa = exchangeBytes(gizY2X(hPa_RATIO,  hPa_ADDITION, dataPoints->valuehPa)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuehPa,hPa_LEN);
        byteFieldOffset += hPa_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagsmoke)
    {
        devStatusTmp.valuesmoke = exchangeBytes(gizY2X(smoke_RATIO,  smoke_ADDITION, dataPoints->valuesmoke)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuesmoke,smoke_LEN);
        byteFieldOffset += smoke_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagCO)
    {
        devStatusTmp.valueCO = exchangeBytes(gizY2X(CO_RATIO,  CO_ADDITION, dataPoints->valueCO)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueCO,CO_LEN);
        byteFieldOffset += CO_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagAQI)
    {
        devStatusTmp.valueAQI = exchangeBytes(gizY2X(AQI_RATIO,  AQI_ADDITION, dataPoints->valueAQI)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueAQI,AQI_LEN);
        byteFieldOffset += AQI_LEN;
    }
